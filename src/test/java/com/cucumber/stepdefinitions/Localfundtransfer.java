...
public class Localfundtransfer {
	...
	public void user_enters_all_fund_tranfer_mandatory(String Testata)
			throws ClassNotFoundException, SQLException, FilloException, ParseException, InterruptedException {

		Hashtable<String, String> amountTranserData = null;
		if (config.DataSource == "DATABASE") {
			amountTranserData = new DatabaseConnector().getDataFromDatabase("LocalTransfer", "DataBinding", Testata);
		} else if (config.DataSource == "FILLO") {
			amountTranserData = new TestData().getCommon_Data("./src/test/resources/database/TestData.xlsx",
					"Databinding", Testata, "LocalFT");
		}
		...
		User.SendKeys(Localpage.receiverAccountNum, amountTranserData.get("ReceAccNum"));
		...
		User.SendKeys(Localpage.dateOfTransfer, amountTranserData.get("DOT"));
		User.SendKeys(Localpage.transferDesc, amountTranserData.get("TransferDesc"));
