...
public class LoginSteps {
	...
	public void user_Launched_in_Browser(String string) {
	...
	public void user_enters_Username_Password(String Databinding, String Sheetname) throws Throwable {
		Hashtable<String, String> exceldata=    new TestData().getCommon_Data("./src/test/resources/database/TestData.xlsx", Databinding, "TestData_001", Sheetname);
        User.SendKeys(LoginPage.UserName, exceldata.get("Username"));
        User.SendKeys(LoginPage.Password,exceldata.get("Password"));
		/*User.SendKeys(LoginPage.UserName, Username);
		User.SendKeys(LoginPage.Password,Password);*/
		Log.info("Username "+exceldata.get("Username")+"and Password are filled with data ");
	}
